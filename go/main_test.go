...
type Test struct {
	subject, formula, output, err string
}
...
func TestNock(t *testing.T) {
	for i, test := range tests {
		...
		subject, err := stringn(test.subject)
		if err != nil {
			t.Fatalf("subject stringn %s", err)
		}

		formula, err := stringn(test.formula)
		if err != nil {
			t.Fatalf("formula stringn %s", err)
		}

		res, err := nock(subject, formula)
		if err != nil && test.err != "" {
			if test.err != err.Error() {
				t.Fatalf("expected %s\ngot %s", test.err, err)
			}
		} else if err == nil && test.output != "" {
			if out := res.String(); test.output != out {
				t.Fatalf("expected %s\ngot %s", test.output, out)
			}
		} else {
			if err != nil && test.output != "" {
				t.Fatalf("expected output %s\ngot failure %s", test.output, err)
			} else if err == nil && test.output != "" {
				t.Fatalf("expected failure %s\ngot output %s", test.err, res)
			} else {
